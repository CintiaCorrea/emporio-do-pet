...
export default function ModelosBoletimTab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setModelos(arr.map((x: any) => { try { const v = JSON.parse(x.valor); return { id: x.id, nome: v.nome || "(sem nome)", texto: v.texto || "" }; } catch { return { id: x.id, nome: "(sem nome)", texto: String(x.valor || "") }; } }));
    } catch { setModelos([]); }
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (form.id) await fetch(`/api/listas/${form.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ valor }) });
      else await fetch("/api/listas", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ lista: "modelo_boletim", valor }) });
      novo(); load();
  ...
  const adicionarSugeridos = async () => {
    ...
    setSalvando(true);
    try { for (const s of SUGERIDOS) await fetch("/api/listas", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ lista: "modelo_boletim", valor: JSON.stringify(s) }) }); load(); }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg px-4 py-3 text-[12.5px]" style={{ background: "#E0F4F6", color: "#00707E" }}>
        Estes textos aparecem na ficha da internação, no bloco <b>🔔 Boletins programados</b>: o vet escolhe o modelo e o texto entra pronto, podendo ajustar antes de enviar. Use <b>[PET]</b> onde entra o nome do animal.
      </div>
      ...
      <div className="bg-white border rounded-[13px]" style={{ borderColor: "#E8E2D6" }}>
        ...
        <div className="p-4 space-y-3">
          ...
          <button onClick={salvar} disabled={salvando} className="text-[12.5px] font-medium text-white bg-[#009AAC] px-4 py-2 rounded-lg disabled:opacity-60">{salvando ? "Salvando…" : form.id ? "Salvar alterações" : "➕ Adicionar modelo"}</button>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-[13px]" style={{ borderColor: "#E8E2D6" }}>
        ...
        {loading ? (
          ...
          <div className="px-4 py-10 text-center"><div className="text-2xl mb-2">📝</div><div className="text-[13px] text-[#5C6B70] mb-1">Nenhum modelo cadastrado ainda.</div><div className="text-[12px] text-[#374151]">Crie o seu acima ou comece pelos sugeridos.</div></div>
        ) : (
