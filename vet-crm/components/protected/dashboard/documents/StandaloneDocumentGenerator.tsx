...
interface StandaloneDocumentGeneratorProps {
  transcription: string;
  onDocumentsGenerated?: (docs: GeneratedDocument[]) => void;
  onBack?: () => void;
}
...
export default function StandaloneDocumentGenerator({
  ...
  const generateDocuments = async () => {
    ...
    if (!transcription) {
      toast.error('É necessário ter uma transcrição para gerar documentos');
      return;
    }
    ...
    if (generated.length > 0) {
      toast.success(`${generated.length} documento(s) gerado(s) com sucesso!`);
      onDocumentsGenerated?.(generated);
    }
    if (errorList.length > 0) {
      toast.error(`${errorList.length} documento(s) falharam na geração`);
    }
  ...
  return (
    <div className="space-y-6">
      {/* Back button and transcription preview */}
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-500">
            {transcription.length} caracteres transcritos
          </span>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={isGenerating || selectedTypes.length === 0 || !transcription}
