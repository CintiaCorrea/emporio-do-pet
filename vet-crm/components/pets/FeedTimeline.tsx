...
const CHIPS: [string, string][] = [["", "Todos"], ["atendimento", "Atendimento"], ["VACCINATION_CARD", "Vacina"], ["PRESCRIPTION", "Receita"], ["EXAM_REQUEST", "Exame"], ["DOC", "Documentos"]];
const CORE = ["atendimento", "VACCINATION_CARD", "PRESCRIPTION", "EXAM_REQUEST"];
...
export default function FeedTimeline({ atendimentos = [], clinDocs = [] }: { atendimentos?: any[]; clinDocs?: any[] }) {
  ...
  const items = useMemo(() => {
    ...
    if (filtro === "DOC") return all.filter((i: any) => !CORE.includes(i.kind));
    return all.filter((i: any) => i.kind === filtro);
  }, [all, filtro]);
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5 mb-3">
        {CHIPS.map(([k, l]) => (
          <button key={k} onClick={() => setFiltro(k)} className="text-[11px] px-2.5 py-1 rounded-full border transition" style={filtro === k ? { background: "#009AAC", color: "#fff", borderColor: "#009AAC" } : { background: "#fff", color: "#475569", borderColor: "#E8DFC8" }}>{l}</button>
        ))}
      </div>
      {items.length === 0 ? (
        <div className="text-sm text-gray-400 py-6 text-center">{all.length === 0 ? "Sem registros no histórico ainda." : "Nenhum registro neste filtro."}</div>
      ) : (
