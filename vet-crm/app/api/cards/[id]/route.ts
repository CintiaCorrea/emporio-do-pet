...
const createCardSchema = z.object({
  ...
  appointmentId: z.string().uuid('ID do agendamento inválido').optional(), // Removed .nullable()
...
export async function GET(request: Request) {
  try {
    ...
    const columnId = searchParams.get('columnId'); // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
export async function POST(request: Request) {
  try {
    ...
    const { columnId, appointmentId, ...restData } = validatedData; // Excluir columnId e appointmentId
    const createData = {
      ...restData,
      column: { connect: { id: validatedData.columnId } },
      ...(validatedData.appointmentId && { appointment: { connect: { id: validatedData.appointmentId } } }),
    };
