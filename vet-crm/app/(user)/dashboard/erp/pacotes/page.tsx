...
const statusInfo: Record<string, { label: string; bg: string; fg: string }> = {
  ...
  CANCELADO: { label: 'Cancelado', bg: '#eef2f3', fg: '#64748b' },
    ...
    <div style={{ width: '100%', background: '#f7f9fa', minHeight: '100%' }}>
      ...
      <div style={{ width: '100%', padding: '20px 26px 60px', boxSizing: 'border-box' }}>
        ...
        <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
          <button onClick={() => setOcultar((v) => !v)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12.5, fontWeight: 500, padding: '8px 12px', borderRadius: 9, cursor: 'pointer', border: '1px solid #d7e0e2', background: '#fff', color: TEAL_DARK }}>
        ...
        <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ width: 266, flex: '0 0 266px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={cardStyle}>
              ...
              <div style={{ fontSize: 12.5, lineHeight: 2 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#0f6e7a' }}>Pacotes ativos</span><b>{resumo.ativos}</b></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#0f6e7a' }}>Sessões restantes</span><b>{resumo.restantes}</b></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#0f6e7a' }}>Valor em ativos</span><b style={{ color: TEAL_DARK }}>{money(resumo.valorAtivo)}</b></div>
              </div>
            </div>
            <div className="no-print" style={cardStyle}>
              ...
              <p style={{ fontSize: 11, color: '#94a3b8', margin: '9px 0 0' }}>A baixa de sessões também é feita ao confirmar o comparecimento do agendamento.</p>
            </div>
          </div>
          ...
          <div style={{ flex: '1 1 480px', minWidth: 0 }}>
            ...
            <div style={{ background: '#fff', border: `1px solid ${LINE}`, borderTop: 'none', borderRadius: '0 0 11px 11px', padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {!loading && lista.length === 0 && <p style={{ color: '#94a3b8', textAlign: 'center', padding: 18 }}>Nenhum pacote nesta aba.</p>}
              {lista.map((p) => {
                ...
                return (
                  <div key={p.id} style={{ border: `1px solid ${LINE}`, borderRadius: 10, padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 600, color: '#1f2d33', fontSize: 14 }}>{p.tutor?.name || 'Cliente'} · {p.pet?.name || 'Pet'}</span>
                      <span style={{ fontSize: 11, fontWeight: 500, padding: '3px 10px', borderRadius: 20, background: st.bg, color: st.fg }}>{st.label}</span>
                    </div>
                    <div style={{ fontSize: 12.5, color: '#64748b', margin: '3px 0 9px' }}>{p.servico}{p.descricao ? ` — ${p.descricao}` : ''} · {money(Number(p.valor))}{p.validade ? ` · validade ${dataLabel(p.validade)}` : ''}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ flex: 1, height: 8, background: '#eef2f3', borderRadius: 20, overflow: 'hidden' }}>
      ...
      {sessaoOpen && sel && (
        <Modal title="Registrar sessão" onClose={() => setSessaoOpen(false)} onConfirm={registrarSessao} confirmLabel="Registrar sessão">
          <div style={{ display: 'flex', justifyContent: 'space-between', background: '#f8fafb', borderRadius: 8, padding: '10px 12px', fontSize: 13 }}>
            <span style={{ color: '#1f2d33' }}>{sel.tutor?.name || 'Cliente'} · {sel.pet?.name || 'Pet'}</span>
            <span style={{ color: '#64748b', fontSize: 12 }}>{sel.sessoesUsadas} de {sel.totalSessoes} feitas</span>
          </div>
...
const inp: React.CSSProperties = { width: '100%', padding: '9px 10px', border: '1px solid #d7e0e2', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', boxSizing: 'border-box' };
