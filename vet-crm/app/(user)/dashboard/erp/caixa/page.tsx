...
import {
  ...
  LuX, LuWallet, LuArrowRightLeft, LuTrash2,
} from 'react-icons/lu';
...
export default function CaixaPage() {
  ...
  const fetchCaixas = useCallback(async () => {
    try {
      ...
      toast.error(e.message || 'Erro ao carregar caixas');
    } finally {
      setLoading(false);
    }
  }, [date, selectedId]);
  ...
  const fetchDetail = useCallback(async (id: string) => {
    try {
      ...
      toast.error(e.message || 'Erro ao carregar caixa');
    }
  }, []);
  ...
  const resumo = useMemo(() => {
    ...
    const tot = linhas.reduce(
      (s, l) => ({ vendas: s.vendas + l.vendas, sup: s.sup + l.sup, resultado: s.resultado + l.resultado }),
  ...
  const abrirCaixa = async () => {
    try {
      const r = await fetch('/api/caixa', {
        ...
        body: JSON.stringify({
          suprimento: Number(String(abrirForm.suprimento).replace(',', '.')) || 0,
          observacao: abrirForm.observacao || null,
        }),
      });
      if (!r.ok) throw new Error('Erro ao abrir caixa');
      toast.success('Caixa aberto!');
      setAbrirOpen(false);
      setAbrirForm({ suprimento: '', observacao: '' });
      await fetchCaixas();
    } catch (e: any) {
  ...
  // ---- Modal receber ----
  const abrirReceber = (venda: Appointment) => {
    setVendaSel(venda);
  ...
  const registrarRecebimento = async () => {
    ...
    try {
      ...
      if (!r.ok) throw new Error('Erro ao registrar recebimento');
  ...
  // Vendas em aberto (para o seletor do modal de recebimento)
  const vendasEmAberto = appointments.filter((a) => {
    const pago = pagoPorAppt.get(a.id) || 0;
    return Number(a.value) - pago > 0.001;
  });
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center border rounded-[9px] overflow-hidden" style={{ borderColor: '#d7e0e2' }}>
              ...
              <span className="text-sm font-medium px-3" style={{ color: '#0f2b32' }}>
                {date === hojeStr() ? 'Hoje · ' : ''}{fmtDataLabel(date)}
              </span>
              <button onClick={() => mudarDia(1)} className="px-3 py-2" style={{ color: TEAL_DARK }} aria-label="Próximo dia"><LuChevronRight size={16} /></button>
            </div>
            <button onClick={() => setAbrirOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-[9px] text-white font-medium" style={{ background: TEAL }}>
              <LuPlus size={16} /> Abrir caixa
            </button>
          </div>
        </div>
            ...
            {caixas.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setSelectedId(c.id)}
                  className="rounded-[10px] px-4 py-2 text-left"
                  style={on
                    ? { border: `1.5px solid ${TEAL}`, background: '#e8f7f9' }
                    : { border: '0.5px solid #d7e0e2', background: '#fff' }}>
                  <div className="text-xs font-medium" style={{ color: on ? '#0f6e7a' : '#475569' }}>
                    Caixa nº {c.numero} · {c.user?.name || '—'}
                  </div>
                  <div className="text-[11px]" style={{ color: on ? '#0f6e7a' : '#94a3b8' }}>
                    {c.status === 'ABERTO' ? `Aberto às ${hora(c.abertura)}` : 'Fechado'}
                  </div>
                </button>
              );
            })}
        ...
        {detail && (
          <div className="bg-white border rounded-xl p-5" style={{ borderColor: '#e1e8ea' }}>
            ...
            <div className="text-sm leading-7 mb-4">
              ...
              <div>
                <span style={{ color: '#0f6e7a', fontWeight: 500 }}>Status:</span>{' '}
                <span className="text-[11px] font-medium px-2.5 py-0.5 rounded-full"
                  style={aberto ? { background: '#e1f5ee', color: '#0f6e56' } : { background: '#eef2f3', color: '#64748b' }}>
              ...
              {(['resumo', 'recebimentos'] as const).map((t) => {
                ...
                return (
                  <button key={t} onClick={() => setTab(t)} className="text-sm pb-2"
                    style={on ? { color: TEAL, fontWeight: 500, borderBottom: `2px solid ${TEAL}` } : { color: '#94a3b8' }}>
                    {t === 'resumo' ? 'Resumo' : 'Lista de recebimentos'}
                  </button>
                );
              })}
            </div>
            ...
            {tab === 'resumo' && (
              <>
                ...
                <table className="w-full text-sm mb-5" style={{ borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {resumo.linhas.length === 0 && (
                      <tr><td colSpan={4} className="py-4 px-2 text-slate-400 text-center">Nenhum valor recebido ainda.</td></tr>
                    )}
                    {resumo.linhas.map((l) => (
                ...
                <div className="text-[15px] mt-6 mb-2" style={{ color: '#0f2b32' }}>Créditos utilizados</div>
                <div className="text-sm text-slate-400 border rounded-lg p-4 text-center" style={{ borderColor: '#eef2f3' }}>
            ...
            {tab === 'recebimentos' && (
              <>
                  ...
                  <div className="mb-3">
                    {vendasEmAberto.length > 0 ? (
                      <details className="text-sm">
                        <summary className="cursor-pointer font-medium" style={{ color: TEAL }}>
                          + Registrar recebimento ({vendasEmAberto.length} venda(s) em aberto)
                        </summary>
                ...
                <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {(detail.recebimentos || []).length === 0 && (
                      <tr><td colSpan={5} className="py-4 px-2 text-slate-400 text-center">Nenhum recebimento registrado.</td></tr>
                    )}
                    {(detail.recebimentos || []).map((rec) => {
                      ...
                      return (
                        <tr key={rec.id} style={{ borderBottom: '0.5px solid #f1f5f6' }}>
                          ...
                          <td className="py-2.5 px-2" style={{ color: '#0f2b32' }}>
                            {rec.appointment?.tutor?.name || 'Cliente'} · {rec.appointment?.pet?.name || 'Pet'}
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-right">
                            <span className="text-[11px] px-2.5 py-0.5 rounded-full" style={{ background: st.bg, color: st.fg }}>{st.label}</span>
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className="flex gap-2 flex-wrap border-t pt-4 mt-5" style={{ borderColor: '#e1e8ea' }}>
              ...
              {aberto && (
                <button onClick={fecharCaixa} className="flex items-center gap-1.5 text-xs font-medium px-3.5 py-2 rounded-lg text-white" style={{ background: TEAL_DARK }}><LuLock size={14} /> Revisar e encerrar</button>
              )}
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input value={abrirForm.suprimento} onChange={(e) => setAbrirForm({ ...abrirForm, suprimento: e.target.value })}
              ...
              <div>
                ...
                <input value={abrirForm.observacao} onChange={(e) => setAbrirForm({ ...abrirForm, observacao: e.target.value })}
                  placeholder="Ex: Abertura de caixa Isabela"
                  className="w-full px-3 py-2.5 border rounded-lg" style={{ borderColor: '#d7e0e2' }} />
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                {formas.map((f, i) => (
                  <div key={i} className="flex gap-1.5 mb-1.5 items-center">
                    <select value={f.forma} onChange={(e) => setFormas(formas.map((x, j) => j === i ? { ...x, forma: e.target.value } : x))}
                      className="flex-[1.4] px-2 py-2 border rounded-lg text-sm" style={{ borderColor: '#d7e0e2' }}>
                      {FORMAS_PADRAO.map((op) => <option key={op} value={op}>{op}</option>)}
                    </select>
                    <input value={f.valor || ''} inputMode="decimal" placeholder="R$"
                      onChange={(e) => setFormas(formas.map((x, j) => j === i ? { ...x, valor: Number(String(e.target.value).replace(',', '.')) || 0 } : x))}
                      className="flex-1 px-2 py-2 border rounded-lg text-sm" style={{ borderColor: '#d7e0e2' }} />
                    <input value={f.parcelas || 1} type="number" min={1} title="Parcelas"
                      onChange={(e) => setFormas(formas.map((x, j) => j === i ? { ...x, parcelas: Number(e.target.value) || 1 } : x))}
                      className="w-14 px-2 py-2 border rounded-lg text-sm text-center" style={{ borderColor: '#d7e0e2' }} />
                    {formas.length > 1 && (
                      <button onClick={() => setFormas(formas.filter((_, j) => j !== i))} aria-label="Remover forma"><LuTrash2 size={15} className="text-slate-400" /></button>
                    )}
                  </div>
                ))}
                <button onClick={() => setFormas([...formas, { forma: 'Pix', valor: 0, parcelas: 1, nsu: '' }])}
                  className="flex items-center gap-1 text-xs font-medium mt-1" style={{ color: TEAL }}><LuPlus size={13} /> adicionar forma</button>
              </div>

              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-xs text-slate-500 mb-1">Desconto</label>
                  <input value={desconto || ''} inputMode="decimal" placeholder="0,00"
                    onChange={(e) => setDesconto(Number(String(e.target.value).replace(',', '.')) || 0)}
                    className="w-full px-2.5 py-2 border rounded-lg text-sm" style={{ borderColor: '#d7e0e2' }} />
                </div>
                <div className="flex-1">
              ...
              <div>
                ...
                <input value={obsReceb} onChange={(e) => setObsReceb(e.target.value)}
                  className="w-full px-2.5 py-2 border rounded-lg text-sm" style={{ borderColor: '#d7e0e2' }} />
              </div>
            </div>
            <div className="flex justify-end gap-3 p-5 border-t" style={{ borderColor: '#eef2f3' }}>
              ...
              <button onClick={registrarRecebimento} className="px-5 py-2.5 rounded-lg text-white font-medium" style={{ background: TEAL }}>Confirmar recebimento</button>
            </div>
          </div>
        </div>
      )}
