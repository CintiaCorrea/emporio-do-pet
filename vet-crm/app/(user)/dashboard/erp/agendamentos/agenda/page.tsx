                      ...
                      <div key={(p._avulsa ? "av-" : "") + p.id} onClick={ocupado ? undefined : () => {
                        ...
                        {cobre.map(({ a, comeca }: any) => { const cor = corDe(a.status, cfg?.cores); const _conf = a.confirmacaoStatus; const terc = terceiroProf[a.id]; const cBorder = terc ? corParceiro(terc.id) : (_conf === "CONFIRMADO" ? "#0F6E56" : _conf === "REMARCAR" ? "#A32D2D" : cor.c); const cBg = _conf === "CONFIRMADO" ? "#E7F7EF" : _conf === "REMARCAR" ? "#FCEBEB" : cor.bg; const v = valorDe(a); const quem = a.pet?.name ? `${a.pet.name}${a.tutor?.name ? ` · ${a.tutor.name}` : ""}` : (a.tutor?.name || "Agendamento");
                          ...
                          <div key={a.id}
                            ...
                            <div className="text-[12px] font-medium truncate hover:underline cursor-pointer flex items-center gap-1" style={{ color: "#014D5E" }} title="Abrir a ficha completa" onClick={(e) => { e.stopPropagation(); const url = a.pet?.id ? `/dashboard/erp/pets/${a.pet.id}` : (a.tutor?.id ? `/dashboard/erp/tutores/${a.tutor.id}` : null); if (url) window.open(url, "_blank"); }}>{/petlife/i.test(a.pet?.insurancePlan || "") && <span title="Convênio Petlife" className="shrink-0">🩺</span>}{a.origem === "PORTAL" && <span title="O próprio tutor marcou pelo Portal do Tutor" className="shrink-0">📱</span>}<span className="truncate">{quem}</span></div>
