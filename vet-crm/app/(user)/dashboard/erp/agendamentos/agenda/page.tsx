                      ...
                      {laid.map(({ a, track, cols }: any) => {
                        ...
                        return (
                          <div key={"nv" + a.id}
                            ...
                            <div className="flex items-center justify-between gap-1">
                              <span className="text-[10.5px] font-bold flex items-center gap-1" style={{ color: cor.c }}>{hm(d)}<span className="font-normal" style={{ opacity: .75 }}>· {dur}min</span>{a.confirmacaoStatus && CONF_BADGE[a.confirmacaoStatus] ? <span title={CONF_BADGE[a.confirmacaoStatus].nome}>{CONF_BADGE[a.confirmacaoStatus].t}</span> : null}{tipoFisio(a) ? (() => { const env = boletinsEnv.has(`${a.pet?.id || a.petId}|${diaStr}`); return <span title={env ? "Boletim de fisio enviado" : "Boletim de fisio ainda não enviado"}>{env ? "🌿✅" : "🌿✉️"}</span>; })() : null}{obsDe(a) ? <span title={obsDe(a) || undefined}>📝</span> : null}</span>
