                  ...
                  return (
                    <div key={(p._avulsa ? "lane-av-" : "lane-") + p.id} className="border-l" style={{ position: "relative", height: totalMinDia * PXMIN, borderColor: "#ECE6D8", overflow: "hidden" }}>
                      ...
                      {laid.map(({ a, track, cols }: any) => {
                        ...
                        const v = valorDe(a);
                        const obs = obsDe(a); // recado da recepção — vira TAG escrita no card (não só ícone)
                        const quem = a.pet?.name ? `${a.pet.name}${a.tutor?.name ? ` · ${a.tutor.name}` : ""}` : (a.tutor?.name || "Agendamento");
                        ...
                        return (
                          <div key={"nv" + a.id}
                            ...
                            <div className="flex items-center justify-between gap-1">
                              <span className="text-[10.5px] font-bold flex items-center gap-1" style={{ color: cor.c }}>{hm(d)}<span className="font-normal" style={{ opacity: .75 }}>· {dur}min</span>{a.confirmacaoStatus && CONF_BADGE[a.confirmacaoStatus] ? <span title={CONF_BADGE[a.confirmacaoStatus].nome}>{CONF_BADGE[a.confirmacaoStatus].t}</span> : null}{tipoFisio(a) ? (() => { const env = boletinsEnv.has(`${a.pet?.id || a.petId}|${diaStr}`); return <span title={env ? "Boletim de fisio enviado" : "Boletim de fisio ainda não enviado"}>{env ? "🌿✅" : "🌿✉️"}</span>; })() : null}{obs ? <span title={obs || undefined}>📝</span> : null}</span>
                            ...
                            {/* Selo de confirmação: NÃO mostra "Confirmado" (já é óbvio pela borda verde + ✅ na hora).
                                Mantém "Aguardando" e "Quer remarcar" — que exigem ação. */}
                            {a.confirmacaoStatus && a.confirmacaoStatus !== "CONFIRMADO" && CONF_BADGE[a.confirmacaoStatus] ? <span className="self-start text-[9px] font-extrabold rounded px-1.5 py-[1px] leading-tight shrink-0" title={CONF_BADGE[a.confirmacaoStatus].nome} style={{ background: CONF_BADGE[a.confirmacaoStatus].c, color: "#fff" }}>{CONF_BADGE[a.confirmacaoStatus].t} {CONF_BADGE[a.confirmacaoStatus].curto}</span> : null}
                            ...
                            {/* Recado da recepção: TAG escrita (não só ícone), logo abaixo do nome, em todo card com observação.
                                "recado que exige passar o mouse não é recado." */}
                            {obs && <div className="text-[9px] truncate px-1 py-0.5 rounded self-start max-w-full" title={obs || undefined} style={{ color: "#8a6400", background: "#FBF3E3" }}>📝 {obs}</div>}
