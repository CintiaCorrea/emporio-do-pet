      ...
      <div style={{ width: '100%', padding: '20px 26px 60px', boxSizing: 'border-box' }}>
        ...
        <div style={{ background: '#fff', border: `1px solid ${LINE}`, borderRadius: 11, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: `1px solid ${LINE}` }}>
            ...
            <span style={{ fontSize: 13, color: '#0f6e7a' }}>Total em crédito <b style={{ color: TEAL_DARK }}>{money(total)}</b></span>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr><th style={th}>Cliente</th><th style={{ ...th, textAlign: 'right' }}>Saldo</th></tr></thead>
              <tbody>
                {loading && <tr><td colSpan={2} style={{ ...td, textAlign: 'center', color: '#94a3b8', padding: 18 }}>Carregando…</td></tr>}
                {!loading && filtradas.length === 0 && <tr><td colSpan={2} style={{ ...td, textAlign: 'center', color: '#94a3b8', padding: 18 }}>Nenhum cliente com saldo.</td></tr>}
                {filtradas.map((r) => (
                  <tr key={r.tutorId}>
                    <td style={{ ...td, color: '#1f2d33' }}>{r.nome}</td>
