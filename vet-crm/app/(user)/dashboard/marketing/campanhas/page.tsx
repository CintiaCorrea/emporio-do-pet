...
export default function CampanhasPage() {
  ...
  const editar = (c: any) => { setEditId(c.id); setForm({ nome: c.nome || "", plataforma: c.plataforma || "Meta Ads", tipo: c.tipo || "Aquisição", segmento: c.segmento || "", metaCampaignId: c.metaCampaignId || "", tagOrigem: c.tagOrigem || "", investimento: c.investimento ?? "", receita: c.receita ?? "", inicio: c.inicio || "", fim: c.fim || "", status: c.status || "Ativa" }); setOpen(true); };
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setOpen(false)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 grid grid-cols-2 gap-3 text-[13px]">
              ...
              <div><label className="text-[11px] text-[#6b7280] block mb-1">Segmento</label><select value={form.segmento} onChange={(e) => setForm({ ...form, segmento: e.target.value })} className="w-full border border-[#d8d0bc] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-[#009AAC]"><option value="">Selecione</option><option value="Clinica">Clinica</option><option value="Fisioterapia">Fisioterapia</option><option value="Integrativa">Medicina Integrativa</option></select></div><div><label className="text-[11px] text-[#6b7280] block mb-1">ID da campanha no Meta</label><input value={form.metaCampaignId} onChange={(e) => setForm({ ...form, metaCampaignId: e.target.value })} className="w-full border border-[#d8d0bc] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-[#009AAC]" /></div><div className="col-span-2"><label className="text-[11px] text-[#6b7280] block mb-1">Tag Origem <span className="text-[#94a3b8]">(liga aos leads — ex.: verao-fisio)</span></label><input value={form.tagOrigem} onChange={(e) => setForm({ ...form, tagOrigem: e.target.value })} className="w-full border border-[#d8d0bc] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-[#009AAC]" /></div>
