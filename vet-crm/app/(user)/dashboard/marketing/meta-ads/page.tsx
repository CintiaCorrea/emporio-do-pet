"use client";
// [EMP-COWORK] Painel Meta Ads (E3) - metricas das campanhas (E2 /api/meta-insights) + segmento (Listas campanha_)
...
export default function MetaAdsPage() {
  usePageTitle("Meta Ads", "Metricas das campanhas: do investimento ao cliente");
  ...
  const rows = useMemo(() => {
    ...
    return cs.map((c) => Object.assign({}, c, { segmento: segMap[c.campaignId] || "Geral" }));
  }, [resp, segMap]);
  ...
  const totals = useMemo(() => {
    const t = { spend: 0, impressions: 0, clicks: 0, conversas: 0 };
    for (const r of filtered as any[]) { t.spend += r.spend; t.impressions += r.impressions; t.clicks += r.clicks; t.conversas += r.conversas; }
    const ctr = t.impressions > 0 ? (t.clicks / t.impressions) * 100 : 0;
    const cpc = t.conversas > 0 ? t.spend / t.conversas : 0;
    return Object.assign({}, t, { ctr, cpc });
  }, [filtered]);
    ...
    <div className="px-4 py-3">
      ...
      <div className="bg-white border border-[#e8dfc8] rounded-xl overflow-x-auto">
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={9} className="text-center text-[#9aa3af] py-6">Carregando...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={9} className="text-center text-[#9aa3af] py-6">Sem campanhas neste segmento/periodo.</td></tr>
              ...
              <tr key={r.campaignId} className="border-t border-[#f0ead9]">
                ...
                <td className="px-2 py-2 text-right text-[#9aa3af]">-</td>
                <td className="px-2 py-2 text-right text-[#9aa3af]">-</td>
                <td className="px-2 py-2 text-right text-[#9aa3af]">-</td>
                <td className="px-2 py-2 text-right text-[#9aa3af]">-</td>
              </tr>
      ...
      <p className="text-[11px] text-[#9aa3af] mt-2">Investido, Conversas e Custo/conv. vem da API do Meta. As colunas do CRM (Leads qual., Vendas, CAC real, ROAS) ligam na etapa E4, cruzando os leads pela campanha de origem.</p>
    </div>
  );
}
